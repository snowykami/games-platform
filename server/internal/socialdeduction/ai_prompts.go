...
func applySocialAIGuidance(state map[string]any, room *Room, player *Player, scope socialDecisionScope) {
	...
	state["playerReferenceRules"] = []string{
		"玩家只应按 seat_编号 或 座位号 理解和称呼；不要把显示名、语言风格、字符类型、真人/AI感当成任何推理依据。",
		"禁止因为某个名字像英文、中文、日文、网名、开发者名或机器人名而怀疑、保护、击杀、投票或组队。",
		"reason 和 speech 都不要写“名字奇怪”“名字偏外”“像真人/像 AI”“这个名字不像普通玩家”等名称相关理由。",
		"若公开发言里有人基于名字带节奏，应把它视为弱理由或可疑带节奏，而不是事实依据。",
	}
...
func socialPhaseGuide(game GameKind, phase string) []string {
	...
	case GameWerewolf:
		...
		case string(PhaseWerewolfNight):
			return []string{
				"夜晚行动通常不需要公开发言；但狼人夜晚的 speech 会进入仅狼队可见的私聊，可用于和队友协商刀口。",
				"狼人必须和所有存活队友选择同一个目标或一起弃权，狼刀才会确认；看到 wolfChoices 不一致时优先沟通并向更合理目标靠拢。",
				"狼人击杀优先考虑威胁高或像神职的目标；也可避开太明显的刀法，或选择弃权制造信息差。",

...
func (h *Handler) Routes() http.Handler {
	...
	router.Post("/rooms/{roomID}/discard", h.discard)
	router.Post("/rooms/{roomID}/self-draw", h.selfDraw)
	router.Post("/rooms/{roomID}/claim", h.claim)
	router.Post("/rooms/{roomID}/skip-claims", h.skipClaims)
	return router
}
...
func (h *Handler) discard(w http.ResponseWriter, r *http.Request) {
	var request discardRequest
	if err := httpx.DecodeJSON(r, &request); err != nil {
		httpx.WriteErrorKey(w, r, http.StatusBadRequest, "invalid_json_body")
		return
	}
	h.mutateRoom(w, r, func(roomID string, userID string) (PublicRoom, error) {
		return h.manager.Discard(roomID, userID, request.TileID)
	})
}

func (h *Handler) selfDraw(w http.ResponseWriter, r *http.Request) {
	h.mutateRoom(w, r, func(roomID string, userID string) (PublicRoom, error) {
		return h.manager.SelfDraw(roomID, userID)
	})
}
...
func (h *Handler) claim(w http.ResponseWriter, r *http.Request) {
	...
	h.mutateRoom(w, r, func(roomID string, userID string) (PublicRoom, error) {
		return h.manager.Claim(roomID, userID, request.ClaimID)
	})
}

func (h *Handler) skipClaims(w http.ResponseWriter, r *http.Request) {
	h.mutateRoom(w, r, func(roomID string, userID string) (PublicRoom, error) {
		return h.manager.SkipClaims(roomID, userID)
	})
}
...
func (h *Hub) handleMessage(message wsMessage, roomID string, userID string) error {
	...
	case "room.discard":
		var request discardRequest
		if err := json.Unmarshal(message.Payload, &request); err != nil {
			return errors.New("invalid_discard_payload")
		}
		_, err := h.manager.Discard(roomID, userID, request.TileID)
		return err
	case "room.self_draw":
		_, err := h.manager.SelfDraw(roomID, userID)
		return err
	case "room.claim":
		var request claimRequest
		if err := json.Unmarshal(message.Payload, &request); err != nil {
			return errors.New("invalid_claim_payload")
		}
		_, err := h.manager.Claim(roomID, userID, request.ClaimID)
		return err
	case "room.skip_claims":
		_, err := h.manager.SkipClaims(roomID, userID)
		return err
	default:
